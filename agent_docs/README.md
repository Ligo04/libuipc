> **Nanobind migration note (2026-09)**: Python bindings live in `src/nanobind`; the public package remains `pyuipc`. CMake uses `UIPC_BUILD_PYTHON_BINDINGS`, and XMake uses `--python_bindings`.
...
| `09-known-issues-and-roadmap.md` | **Open issues and plans**: current four-scene performance envelope, historical optimization ledger, deferred CFL floor, dependency pins, external PR review, and samples state |
...
| [`14-project-audit.md`](14-project-audit.md) | 2026-09-11 inspection: reproduced API/integration issues, frontend contracts, documentation corrections, validation limits and prioritized proposals; no implementation changes |
...
- **libuipc** = a C++20 cross-platform GPU physics simulation library implementing Unified IPC (Incremental Potential Contact), unifying simulation of rigid bodies/soft bodies/cloth/rods with frictional contact. Non-penetration relies on valid initial geometry and successful collision/solve steps, not arbitrary invalid inputs. Corresponding papers: GIPC 2024, StiffGIPC 2025 (Siggraph).
...
- Tests: Catch2 (`apps/tests/`, including 95 isolated sim cases) + pytest
  (`python/tests/`, 24 tracked files) + repository/Blender helper tests. Parametrized
  runtime counts are not source-function counts; see doc 14 for dated results.
...
## Known Documentation Drift Notes (as of 2026-09)
...
- `docs/development/deterministic_mode.md` is a historical, withdrawn proposal,
  not an accepted implementation task or a supported configuration flag. Rule 16
  prohibits restructuring parallel execution merely to eliminate atomic-order rounding.
- The latest cross-cutting inspection is [doc 14](14-project-audit.md). It separates
  reproduced/source-confirmed problems from risks and optimization opportunities;
  its listed implementation fixes have not been applied.
- `libuipc-samples/` is a tracked submodule with 53 current example directories; numbering is historical and includes two `40_*` directories.
- Incremental Scene commits are lossless relative to a compatible full baseline,
  including removals, sparse IDs, and contact/subscene tables. They are not
  standalone Scene files; read doc 11 for baseline and lifecycle requirements.
