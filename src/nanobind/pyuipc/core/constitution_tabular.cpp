...
PyConstitutionTabular::PyConstitutionTabular(py::module_& m)
{
    ...
    class_ConstitutionTabular.def(py::init<>(), R"(Create an empty constitution tabular.)");
...
Args:
    ...
    class_ConstitutionTabular.def(
        ...
        { return as_numpy(self.uids(), py::find(&self)); },
        R"(Get the UIDs of all constitutions.
Returns:
    numpy.ndarray: Array of constitution UIDs.)");

    class_ConstitutionTabular.def("types",
                                  &ConstitutionTabular::types,
