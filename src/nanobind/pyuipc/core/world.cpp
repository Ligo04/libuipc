...
Args:
    ...
    class_World.def("advance",
                    &World::advance,
                    py::call_guard<py::gil_scoped_release>(),
                    R"(Advance the simulation by one step.)");
    class_World.def("sync", &World::sync, py::call_guard<py::gil_scoped_release>(), R"(Synchronize the world state.)");
    class_World.def("retrieve",
                    &World::retrieve,
                    py::call_guard<py::gil_scoped_release>(),
                    R"(Retrieve the world state.)");
    class_World.def("dump", &World::dump, py::call_guard<py::gil_scoped_release>(), R"(Dump the world state.)");
