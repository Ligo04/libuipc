...
PyEngine::PyEngine(py::module_& m)
{
    ...
    // Keep the legacy keyword-named member while providing a Python-safe alias.
    // Assign the existing object instead of registering the C++ value twice:
    // nanobind otherwise creates two distinct Python enum objects for value 0.
    enum_Type.attr("None_")          = enum_Type.attr("None");
    class_EngineStatus.attr("None_") = enum_Type.attr("None");
...
Returns:
    Status type enumeration (None_, Info, Warning, or Error).)")
        .def("what",
             &EngineStatus::what,
...
Returns:
    ...
    auto class_EngineStatusCollection = py::class_<EngineStatusCollection>(
    ...
    class_EngineStatusCollection.def(
        "push_back",
...
Args:
    ...
    class_EngineStatusCollection.def("has_error",
                                     &EngineStatusCollection::has_error,
...
Returns:
    ...
    auto class_Engine =
        py::class_<Engine>(m, "Engine", R"(Engine class for running simulations with a specific backend.)");
