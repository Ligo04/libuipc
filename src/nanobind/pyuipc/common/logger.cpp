...
PyLogger::PyLogger(py::module_& m)
{
    ...
    auto enum_LoggerLevel = py::enum_<spdlog::level::level_enum>(
        class_Logger, "Level", R"(Logging level enumeration.)");
    ...
    class_Logger.def_static(
        "set_level",
        [](spdlog::level::level_enum level) { uipc::logger::set_level(level); },
        py::arg("level"),
...
Args:
    ...
    class_Logger.def_static(
        "set_pattern",
        [](std::string_view pattern) { uipc::logger::set_pattern(pattern); },
        py::arg("pattern"),
...
Args:
    ...
    class_Logger.def_static(
        "debug",
        [](std::string_view msg) { uipc::logger::debug(msg); },
        py::arg("msg"),
...
Args:
    ...
    class_Logger.def_static(
        "error",
        [](std::string_view msg) { uipc::logger::error(msg); },
        py::arg("msg"),
...
Args:
    ...
    class_Logger.def_static(
        "critical",
        [](std::string_view msg) { uipc::logger::critical(msg); },
        py::arg("msg"),
