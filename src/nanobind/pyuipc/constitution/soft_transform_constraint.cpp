...
Returns:
    ...
    auto class_RotatingMotor = py::class_<RotatingMotor, Constraint>(
        m, "RotatingMotor", R"(RotatingMotor constraint for applying rotational motion to affine bodies.)");

    class_RotatingMotor
        .def(py::init<const Json&>(),
             py::arg("config") = RotatingMotor::default_config(),
...
Args:
    ...
    motor_rot_vel: Rotational velocity in rad/s (default: 2π).)")
        .def_static("default_config",
                    &RotatingMotor::default_config,
                    R"(Get the default RotatingMotor configuration.
Returns:
    dict: Default configuration dictionary.)")
        .def_static("animate",
                    &RotatingMotor::animate,
                    py::arg("sc"),
                    py::arg("dt"),
...
Args:
    ...
    auto class_LinearMotor = py::class_<LinearMotor, Constraint>(
        m, "LinearMotor", R"(LinearMotor constraint for applying linear motion to affine bodies.)");

    class_LinearMotor
        .def(py::init<const Json&>(),
             py::arg("config") = LinearMotor::default_config(),
