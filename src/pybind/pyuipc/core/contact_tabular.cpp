...
{
    {
        auto class_ContactElement = py::class_<ContactElement>(
            m, "ContactElement", R"(ContactElement class representing a contact element (object or geometry).)");
...
Args:
    id: Element ID.
    name: Element name.)");
        class_ContactElement.def("id",
                                 &ContactElement::id,
                                 R"(Get the element ID.
Returns:
    int: Element ID.)");
        class_ContactElement.def("name",
                                 &ContactElement::name,
                                 R"(Get the element name.
Returns:
    str: Element name.)");
        class_ContactElement.def("apply_to",
                                 &ContactElement::apply_to,
                                 py::arg("object"),
...
Returns:
    float: Friction rate value.)");
        class_ContactModel.def("resistance",
                               &ContactModel::resistance,
                               R"(Get the resistance value.
Returns:
    float: Resistance value.)");
        class_ContactModel.def("is_enabled",
                               &ContactModel::is_enabled,
                               R"(Check if the contact model is enabled.
Returns:
    bool: True if enabled, False otherwise.)");
        class_ContactModel.def("config",
                               &ContactModel::config,
...
Returns:
    ...
    {
        auto class_ContactModelCollection = py::class_<ContactModelCollection>(
            m, "ContactModelCollection", R"(Collection of contact models with attributes.)");
...
Returns:
    AttributeSlot: Created attribute slot.)");
        ...
        class_ContactModelCollection.def(
            "find",
            [](ContactModelCollection& self, std::string_view name) -> S<uipc::geometry::IAttributeSlot>
...
Returns:
    ...
    {
        auto class_ContactTabular = py::class_<ContactTabular>(
            m, "ContactTabular", R"(ContactTabular class managing contact elements and contact models between them.)");

        // Elements:
        class_ContactTabular.def("create",
                                 &ContactTabular::create,
                                 py::arg("name") = "",
...
Returns:
    ContactElement: Reference to default element.)");
        ...
        class_ContactTabular.def("element_count",
                                 &ContactTabular::element_count,
