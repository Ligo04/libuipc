...
PyAffineBodyRod::PyAffineBodyRod(py::module_& m)
{
    auto class_AffineBodyRod =
        py::class_<AffineBodyRod, AffineBodyConstitution>(m, "AffineBodyRod", R"(Codimensional 1D (rod) affine body constitution.
...
treated as thin rods with a circular cross-section of a given radius.)");
    ...
    class_AffineBodyRod.def(py::init<const Json&>(),
                            py::arg("config") = AffineBodyRod::default_config(),
                            R"(Create an AffineBodyRod.
Args:
    config: Configuration dictionary (optional, uses default if not provided).)");

    class_AffineBodyRod.def("apply_to",
