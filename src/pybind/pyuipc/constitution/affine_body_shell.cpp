...
PyAffineBodyShell::PyAffineBodyShell(py::module_& m)
{
    auto class_AffineBodyShell =
        py::class_<AffineBodyShell, AffineBodyConstitution>(m, "AffineBodyShell", R"(Codimensional 2D (shell) affine body constitution.
...
treated as thin shells with a given thickness.)");
    ...
    class_AffineBodyShell.def(py::init<const Json&>(),
                              py::arg("config") = AffineBodyShell::default_config(),
                              R"(Create an AffineBodyShell.
Args:
    config: Configuration dictionary (optional, uses default if not provided).)");

    class_AffineBodyShell.def("apply_to",
