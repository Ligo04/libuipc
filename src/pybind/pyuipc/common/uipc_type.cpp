...
void def_scalar(py::module_& m, const char* name)
{
    ...
    auto class_Scalar = py::class_<Ty>(m, name, R"(Type wrapper for scalar values.)");

    class_Scalar.def_static(
        "Zero",
        []() { return VT{0}; },
        R"(Get zero value.
Returns:
...
void def_matrix(py::module_& m, Eigen::Matrix<T, M, N>, const char* name)
{
    ...
    auto class_Matrix = py::class_<Ty>(m, name, R"(Type wrapper for matrix values.)");
...
Returns:
    ...
    class_Matrix.def_static(
        ...
        []()
        {
            Eigen::Matrix<T, M, N> mat = Eigen::Matrix<T, M, N>::Ones();
            return as_numpy(mat);
        },
...
Returns:
    ...
    class_Matrix.def_static(
        ...
        []()
        {
            Eigen::Matrix<T, M, N> mat = Eigen::Matrix<T, M, N>::Identity();
            return as_numpy(mat);
        },
...
Returns:
    ...
    class_Matrix.def_static(
        ...
        []()
        {
            Eigen::Matrix<T, M, N> mat = Eigen::Matrix<T, M, N>::Random();
            return as_numpy(mat);
        },
    ...
    numpy.ndarray: Unit vector.)");
        ...
        if constexpr(M >= 1)
        {
            class_Matrix.def_static(
                ...
                []()
                {
                    Eigen::Matrix<T, M, N> mat = Eigen::Matrix<T, M, N>::UnitX();
                    return as_numpy(mat);
                },
    ...
    numpy.ndarray: Unit X vector.)");
        ...
        if constexpr(M >= 2)
        {
            class_Matrix.def_static(
                ...
                []()
                {
                    Eigen::Matrix<T, M, N> mat = Eigen::Matrix<T, M, N>::UnitY();
                    return as_numpy(mat);
                },
    ...
    numpy.ndarray: Unit Y vector.)");
        ...
        if constexpr(M >= 3)
        {
            class_Matrix.def_static(
                ...
                []()
                {
                    Eigen::Matrix<T, M, N> mat = Eigen::Matrix<T, M, N>::UnitZ();
                    return as_numpy(mat);
                },
    ...
    numpy.ndarray: Unit Z vector.)");
        ...
        if constexpr(M >= 4)
        {
            class_Matrix.def_static(
                ...
                []()
                {
                    Eigen::Matrix<T, M, N> mat = Eigen::Matrix<T, M, N>::UnitW();
                    return as_numpy(mat);
                },
