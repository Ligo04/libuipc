...
PyUrdfIO::PyUrdfIO(py::module_& m)
{
    ...
    class_UrdfController.def("rotate_to",
                             &UrdfController::rotate_to,
                             py::arg("joint_name"),
                             py::arg("angle"),
...
Args:
    ...
    class_UrdfController.def("apply_to",
                             &UrdfController::apply_to,
                             py::arg("attr"),
                             R"(Apply controller state to attributes.
Args:
    attr: Attribute collection to apply to.)");
    class_UrdfController.def("revolute_joints",
                             &UrdfController::revolute_joints,
...
Returns:
    ...
    class_UrdfController.def("sync_visual_mesh",
                             &UrdfController::sync_visual_mesh,
                             R"(Synchronize visual mesh with current joint positions.)");
    class_UrdfController.def(
        "move_root",
        [](const UrdfController& self, PyArray<Float> xyz, PyArray<Float> rpy)
        {
...
Args:
    ...
    auto class_UrdfIO =
        py::class_<UrdfIO>(m, "UrdfIO", R"(UrdfIO class for loading URDF robot models.)");
    class_UrdfIO.def(py::init<const Json&>(),
                     py::arg("config") = UrdfIO::default_config(),
                     R"(Create a UrdfIO instance.
Args:
    config: Configuration dictionary (optional, uses default if not provided).)");
    class_UrdfIO.def("read",
                     &UrdfIO::read,
                     py::arg("object"),
                     py::arg("urdf_path"),
