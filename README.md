...
With `UIPC_BUILD_PYTHON_BINDINGS=ON`, the nanobind extension is built **and installed** into the active Python environment. The former `UIPC_BUILD_PYBIND` option remains a deprecated compatibility alias. If the install step fails, finish it manually with `cd build/python && pip install .`; to target a specific venv, pass `-DUIPC_PYTHON_EXECUTABLE_PATH=<python.exe>`.
...
**XMake alternative**: `xmake f -c && xmake build -j8` (enable the nanobind extension with `xmake f --python_bindings=true --python_system=true --python_version=3.11.x -c`; the former `--pybind` option remains a deprecated compatibility alias).
