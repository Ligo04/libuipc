...
## Embedded Source Dependencies

| Name | Location | Purpose |
| --- | --- | --- |
| METIS 5.2.1 C++ implementation | `src/geometry/metis/` | Private mesh/graph partitioning target used by `uipc_geometry` |
| GKlib-derived support subset | `src/geometry/metis/` | Private support code compiled into the embedded METIS target |

The project no longer builds separate `external/METIS` or `external/GKlib`
targets. Upstream Apache-2.0 notices are stored with the embedded
implementation. The port omits unused glibc-derived getopt/regex/qsort
compatibility code and the optional third-party RNG. It uses an in-tree
deterministic C++ partition/insertion sort that preserves the former equal-key
ordering, plus the existing C-runtime random path.
...
See `pyproject.toml` / `python/pyproject.toml`: `nanobind 3.0.1` builds the native
